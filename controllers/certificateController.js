...
exports.issueCertificate = async (req, res, next) => {
    ...
    const userId = req.user.email; // Using email as identity for the cert

    if (!csrPem) return res.status(400).json({ error: 'Missing csrPem' });
    ...
    try {
        ...
        if (!cn || cn.value !== userId) {
            return res.status(400).json({ error: 'CSR Common Name (CN) must match user email' });
        }
        ...
        if (issued[userId]?.certPem) {
            try {
                ...
                // If parsing fails, proceed to reissue
            }
        }
        ...
        console.error("Certificate Issuance Error:", e.message);
